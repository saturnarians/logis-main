...
interface LogisticsContextType {
  ...
  createShipment: (
    shipment: Omit<
      Shipment,
      "id" | "createdAt" | "timeline" | "routePoints" | "currentLocation"
    >,
  ) => void;
  ...
  toggleAIReadPermission: (
    key: keyof AIGovernanceConfig["readPermissions"],
  ) => void;
  toggleAIWritePermission: (
    key: keyof AIGovernanceConfig["writePermissions"],
  ) => void;
  ...
  executeOrQueueAIAction: (
    action: Omit<PendingAIAction, "id" | "status" | "timestamp">,
  ) => {
...
export const LogisticsProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  ...
  const [warehouses, setWarehouses] =
    useState<WarehouseHub[]>(INITIAL_WAREHOUSES);
  ...
  const [permissions, setPermissions] =
    useState<SystemPermission[]>(INITIAL_PERMISSIONS);
  const [aiGovernance, setAiGovernance] = useState<AIGovernanceConfig>(
    INITIAL_AI_GOVERNANCE,
  );
  ...
  const loginUser = (
    ...
    const matchedUser = usersList.find(
      (u) =>
        ...
        ((credentials?.email &&
          ...
          (credentials?.staffId &&
            u.staffId.toLowerCase() === credentials.staffId.toLowerCase())),
    ) ||
      usersList.find((u) => u.role === targetRole) || {
        ...
        staffId:
          credentials?.staffId ||
          `DHL-ID-${Math.floor(1000 + Math.random() * 9000)}`,
        hub: credentials?.hub || "London Central Gateway",
        vehicleId:
          credentials?.vehicleId ||
          (targetRole === "driver" ? "DHL-EV-402" : undefined),
        department:
          targetRole === "superadmin"
            ? "Executive Logistics Command"
            : targetRole === "admin"
              ? "Dispatch Operations"
              : "Express Delivery",
    ...
    if (targetRole === "driver") {
      const matchedDriver = drivers.find((d) =>
        d.name.toLowerCase().includes(matchedUser.name.toLowerCase()),
      );
      if (matchedDriver) {
  ...
  const [activeNavSection, setActiveNavSection] =
    useState<NavSection>("overview_dashboard");
  ...
  const [trackedShipmentId, setTrackedShipmentId] = useState<string | null>(
    "DHL-8942-01",
  );
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (json.success && json.data) {
          if (json.data.shipments && json.data.shipments.length > 0)
          ...
          if (json.data.alerts && json.data.alerts.length > 0)
            setAlerts(json.data.alerts);
          if (json.data.invoices && json.data.invoices.length > 0)
            setInvoices(json.data.invoices);
          if (json.data.tariffs && json.data.tariffs.length > 0)
            setTariffs(json.data.tariffs);
          if (json.data.inventory && json.data.inventory.length > 0)
            setInventory(json.data.inventory);
          if (json.data.permissions && json.data.permissions.length > 0)
  ...
  const reseedDatabase = async (): Promise<{
    success: boolean;
    message: string;
  }> => {
    try {
      ...
      if (json.success) {
        ...
        return {
          success: true,
          message: json.message || "Database reseeded successfully!",
        };
      }
      return {
        success: false,
        message: json.error || "Failed to reseed database",
      };
    } catch (err: any) {
      return {
        success: false,
        message: err.message || "Network error while reseeding",
      };
    }
  };
      ...
      setShipments((prevShipments) =>
        prevShipments.map((shipment) => {
          if (
            ...
            const newLat = Number(
              (shipment.currentLocation.lat + deltaLat).toFixed(4),
            );
            const newLng = Number(
              (shipment.currentLocation.lng + deltaLng).toFixed(4),
            );
        ...
        if (s.id === id || s.trackingId === id) {
          ...
          return {
            ...
            currentLocation: location
              ? { ...s.currentLocation, address: location }
              : s.currentLocation,
            timeline: [
              newTimelineEntry,
              ...s.timeline.filter((entry) => entry.status !== newStatus),
            ],
            proofOfDelivery:
              isDelivered && !s.proofOfDelivery?.verified
                ? {
                    verified: true,
                    verifiedAt: timestamp,
                    signedByCustomerName: s.customerName,
                  }
                : s.proofOfDelivery,
          };
        }
      ...
      fetch("/api/shipments", {
        ...
        body: JSON.stringify({
          ...
          timelineEntry: {
            ...
            location:
              location ||
              targetShipment?.currentLocation.address ||
              "Transit Depot",
            timestamp,
            note: note || `Status updated to ${newStatus}`,
            updatedBy: "DHL",
  ...
  const createShipment = (
    ...
    const trackingId =
      newShipmentData.trackingId ||
      `DHL-${Math.floor(1000 + Math.random() * 9000)}-${Math.floor(10 + Math.random() * 90)}`;
    const orderId =
      newShipmentData.orderId ||
      ...
      routePoints: [
        {
          lat: 51.5074,
          lng: -0.1278,
          name: `${newShipmentData.originCity} Central Depot`,
          completed: true,
          timestamp,
        },
        {
          lat: 51.52,
          lng: -0.09,
          name: `${newShipmentData.destinationCity} Destination`,
          completed: false,
        },
      ],
      timeline: [
      ...
      setDrivers((prev) =>
        prev.map((d) =>
          d.id === driver.id
            ? {
                ...d,
                assignedShipmentIds: [...d.assignedShipmentIds, id],
                status: "On Duty",
              }
            : d,
        ),
    ...
    try {
      fetch("/api/shipments", {
        ...
        body: JSON.stringify({
          ...
          senderEmail: newShipmentData.senderEmail,
          senderPhone: newShipmentData.senderPhone,
          senderAddress:
            newShipmentData.senderAddress ||
            `${newShipmentData.originCity} Logistics Depot`,
          recipientEmail: newShipmentData.recipientEmail,
  ...
  const submitProofOfDelivery = (shipmentId: string, pod: ProofOfDelivery) => {
    ...
    const targetShipment = shipments.find(
      (s) => s.id === shipmentId || s.trackingId === shipmentId,
    );
    if (targetShipment?.driverId) {
      setDrivers((prev) =>
        prev.map((d) =>
          d.id === targetShipment.driverId
            ? { ...d, completedToday: d.completedToday + 1 }
            : d,
        ),
      );
    }
      ...
      prev.map((inv) =>
        inv.id === invoiceId
          ? {
              ...
              status,
              paidAt:
                status === "Paid" || status === "COD Collected"
                  ? timestamp
                  : inv.paidAt,
            }
          : inv,
  ...
  const toggleAIMasterSwitch = (enabled: boolean) => {
    ...
    const actorName =
      currentUser?.name ||
      (role === "superadmin" ? "Superadmin Executive" : "Superadmin");
  ...
  const approveAIAction = (actionId: string) => {
    ...
    if (!action)
      return { success: false, message: "Action not found in queue." };
    ...
    if (
      action.actionType === "update_shipment_status" &&
      ...
      action.actionType === "assign_driver" &&
      action.payload.shipmentId &&
      ...
      action.actionType === "create_route_alert" &&
    ...
    return {
      success: true,
      message: `Successfully approved & executed: ${action.title}`,
    };
  };
  ...
  const executeOrQueueAIAction = (
    actionData: Omit<PendingAIAction, "id" | "status" | "timestamp">,
  ) => {
    const timestamp = new Date()
      .toISOString()
      .replace("T", " ")
    ...
    if (!aiGovernance.aiEnabled) {
      return {
        ...
        message: "AI is currently turned OFF by SuperAdmin.",
      };
    }
    ...
    if (aiGovernance.dataAccessMode === "read_only") {
      return {
        status: "blocked" as const,
        message:
          "AI Write Access is blocked under Read-Only Mode. Human approval is required.",
      };
    }
    ...
    if (aiGovernance.dataAccessMode === "read_write") {
      ...
      if (
        actionData.actionType === "update_shipment_status" &&
        ...
        actionData.actionType === "assign_driver" &&
        actionData.payload.shipmentId &&
        actionData.payload.driverId
      ) {
        assignDriverToShipment(
          actionData.payload.shipmentId,
          actionData.payload.driverId,
        ...
        actionData.actionType === "create_route_alert" &&
        actionData.payload.alertMessage
      ) {
        const newAlert: LogisticsAlert = {
          id: `alt-${Date.now()}`,
        ...
        actionData.actionType === "update_tariff" &&
        actionData.payload.tariffId
      ) {
      ...
      return {
        ...
        message: `Directly executed: ${actionData.title}`,
        action: pendingItem,
      };
    }
    ...
    return {
      ...
      message: `Drafted and queued for SuperAdmin approval: ${actionData.title}`,
      action: pendingItem,
    };
  };
